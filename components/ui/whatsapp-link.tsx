...
/**
 * El logo de WhatsApp: lucide no porta icones de marca. Mateix criteri que
 * `YouTubeMark`.
 *
 * ELS ESPAIS DEL `d` NO SÓN DECORATIUS
 *
 * En un `d` els nombres se separen per espai NOMÉS quan no es poden distingir
 * sols: `-.27.3` ja són dos, però `2.47` i `0` seguits necessiten l'espai o
 * es llegeixen com un sol `2.470`. Això és exactament el que va passar aquí:
 * un `0` es va enganxar al nombre de davant, cada grup de la corba va quedar
 * desplaçat un lloc i el navegador rebutjava l'atribut sencer —la icona es
 * dibuixava a mitges i la consola en donava constància a cada fitxa de client.
 *
 * Per això no s'hi passa cap formatador ni s'hi reparteixen salts de línia.
 */
function WhatsAppMark({ className = "h-4 w-4" }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" className={className} aria-hidden>
      <path
        fill="#25D366"
        d="M17.47 14.38c-.3-.15-1.75-.86-2.02-.96-.27-.1-.47-.15-.67.15-.2.3-.77.96-.94 1.16-.17.2-.35.22-.64.07-.3-.15-1.25-.46-2.38-1.47-.88-.78-1.47-1.75-1.65-2.05-.17-.3-.02-.46.13-.6.13-.14.3-.35.45-.52.15-.17.2-.3.3-.5.1-.2.05-.37-.02-.52-.08-.15-.67-1.6-.92-2.2-.24-.58-.49-.5-.67-.51h-.57c-.2 0-.52.07-.79.37-.27.3-1.04 1.01-1.04 2.47 0 1.5 1.08 2.5 1.23 2.68.15.2 2.1 3.2 5.08 4.49.71.3 1.26.49 1.69.63.71.22 1.36.19 1.87.12.57-.09 1.75-.72 2-1.41.25-.69.25-1.28.17-1.41-.07-.13-.27-.2-.57-.35z"
      />
      <path
        fill="#25D366"
